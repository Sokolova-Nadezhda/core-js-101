...
function doRectanglesOverlap(rect1, rect2) {
  const xOverlap = rect1.left <= rect2.left + rect2.width && rect1.left + rect1.width >= rect2.left;
  const yOverlap = rect1.top <= rect2.top + rect2.height && rect1.top + rect1.height >= rect2.top;
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    frequencyMap.set(str[i], (frequencyMap.get(str[i]) || 0) + 1);
  }
...
function isCreditCardNumber(ccn) {
  ...
  const ccnDigits = ccnStr.split('').map(Number);

  let sum = 0;
  for (let i = ccnDigits.length - 2; i >= 0; i -= 2) {
  ...
  for (let i = 0; i < ccnDigits.length; i += 1) {
    sum += ccnDigits[i];
  }
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (openBrackets.includes(str[i])) {
      ...
      const correspondingOpenBracket = openBrackets[closeBrackets.indexOf(str[i])];
...
function evaluateTicTacToePosition(position) {
  ...
  if (
    position[0][0] && position[0][0] === position[1][1] && position[1][1] === position[2][2]
  ) {
    return position[0][0];
  }

  if (
    position[0][2] && position[0][2] === position[1][1] && position[1][1] === position[2][0]
  ) {
    return position[0][2];
  }
